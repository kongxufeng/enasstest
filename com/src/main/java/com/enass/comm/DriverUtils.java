...
public class DriverUtils {
	 ...
	 */
	static {
		browser = System.getProperty("test.browser", "firefox"); // 获取bwf.test.browser系统属性，如果未设置，则默认为firefox
		/*
		...
		if("firefox".equalsIgnoreCase(browser)) {
			service = new GeckoDriverService.Builder()
				.usingAnyFreePort() // 指定使用任一闲置端口
					.usingDriverExecutable(new File("C:/Program Files/geckodriver.exe")) // 指定driver驱动程序的位置
				.build(); // 将Builder对象打造成DriverService对象
		}else if("chrome".equalsIgnoreCase(browser)) {
			service = new ChromeDriverService.Builder()
					.usingDriverExecutable(new File("C:/Program Files (x86)/chromedriver.exe"))
					.usingAnyFreePort() // 指定使用任一闲置端口
					.build();
